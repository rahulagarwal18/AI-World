        ...
        <script>
            ...
            async function previewFile(filename) {
                ...
                try {
                    ...
                    // If file is HTML, load directly into iframe
                    if (filename.endsWith('.html')) {
                        iframe.src = previewUrl;
                        iframe.srcdoc = "";
                    } else {
                        // For .py, .json, .txt files, render a Rich Graphical App Visualizer Card inside iframe srcdoc
                        const isPython = filename.endsWith('.py');
                        const isJson = filename.endsWith('.json');
                        const fileSizeKB = (text.length / 1024).toFixed(2);
                        const linesCount = text.split('\n').length;
                        
                        // Extract docstrings or description
                        let description = "Autonomous Python Script generated by Groq AI model.";
                        const docMatch = text.match(/\"\"\"([\s\S]*?)\"\"\"/) || text.match(/# (.*)/);
                        if (docMatch) {
            ...
            async function loadData() {
                try {
                    ...
                    if (dataHist.history && dataHist.history.length > 0) {
                        ...
                        actionsDiv.innerHTML = dataHist.history.slice().reverse().map(function(item) {
                            var act = item.action || {};
                            var actType = act.action || 'ACTION';
                            var badgeColor = 'bg-cyan-500/20 text-cyan-400 border-cyan-500/30';
                            ...
                            var timeStr = new Date(item.timestamp * 1000).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit', second:'2-digit'});
                            var thoughtStr = act.thought || 'Autonomous action executed.';
                            var pathBtn = act.path ? '<button onclick="previewFile(\'' + act.path + '\')" class="w-full text-left bg-slate-900/80 hover:bg-slate-900 px-3 py-1.5 rounded-xl text-[11px] mono text-cyan-300 border border-slate-800 flex items-center justify-between group transition"><span>📄 ' + act.path + '</span><span class="text-[10px] text-slate-500 group-hover:text-cyan-400">View App →</span></button>' : '';

                            return '<div class="glass-card p-3.5 rounded-2xl space-y-2 hover:border-slate-600 transition">'
                                + '<div class="flex justify-between items-center">'
                                + '<span class="px-2.5 py-0.5 rounded-md text-[10px] font-bold uppercase border ' + badgeColor + '">' + actType + '</span>'
                                + '<span class="text-[10px] mono text-slate-400">' + timeStr + '</span></div>'
                                + '<p class="text-xs text-slate-200 leading-relaxed font-medium">' + thoughtStr + '</p>'
                                + pathBtn + '</div>';
                        }).join('');
                    } else {
                        streamCount.innerText = '0 Actions';
                        actionsDiv.innerHTML = '<div class="text-center py-10 space-y-2"><p class="text-slate-400 text-xs font-semibold">24/7 Creation Loop Active</p><p class="text-slate-500 text-[11px]">Groq model is analyzing and inventing world files...</p></div>';
                    }
                    ...
                    if (dataFiles.files && dataFiles.files.length > 0) {
                        ...
                        filesDiv.innerHTML = dataFiles.files.map(function(f) {
                            var isSelected = f === currentSelectedFile;
                            var activeClass = isSelected ? 'border-cyan-500/50 bg-cyan-500/10 text-cyan-300' : 'border-slate-800 bg-slate-900/40 text-slate-300 hover:bg-slate-800/60';
                            return '<button onclick="previewFile(\'' + f + '\')" class="w-full text-left p-3 rounded-xl border transition flex items-center justify-between group ' + activeClass + '">'
                                + '<div class="flex items-center gap-2.5 truncate"><span class="text-sm">📄</span><span class="text-xs mono font-medium truncate">' + f + '</span></div>'
                                + '<span class="text-[11px] text-cyan-400 font-medium opacity-80 group-hover:opacity-100">View →</span></button>';
                        }).join('');
